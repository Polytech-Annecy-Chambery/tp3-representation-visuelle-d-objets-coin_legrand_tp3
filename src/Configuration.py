...
class Configuration:
	...
	def processKeyDownEvent(self):
		...
		elif self.event.key == 1073741899: # pageup
			gl.glMatrixMode(gl.GL_MODELVIEW)
			gl.glScalef(1.1, 1.1, 1.1)
		elif self.event.key == 1073741902: # pagedown
			gl.glMatrixMode(gl.GL_MODELVIEW)
	...
	def processMouseMotionEvent(self):
		...
		if mouseState[0]: # gauche enfoncé, rotation
			gl.glMatrixMode(gl.GL_MODELVIEW)
			gl.glRotatef(self.event.rel[1]*0.5, 1, 0, 0)
			gl.glRotatef(self.event.rel[0]*0.5, 0, 0, 1)
		if mouseState[2]: # droit enfoncé, translation
			gl.glMatrixMode(gl.GL_MODELVIEW)
